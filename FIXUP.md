...
1. -Wunused-function build: No additional unused functions found (page_size_string was the only one)

2. Large library files checked:
   - flex_proportions.c (257 LOC): Used by page-writeback.c and backing-dev.c - cannot remove
   - string_helpers.c (775 LOC): Previously checked, functions used despite no nm symbols

3. Header analysis:
   - cpumask.h (1,044 LOC): 413 lines are comments (40%)
   - atomic-arch-fallback.h (2,456 LOC): Generated by script, tricky to modify
   - mod_devicetable.h (914 LOC): Only used in scripts/mod, potentially stubbable but risky

4. Binary analysis (nm -S --size-sort vmlinux):
   - accent_table (0xc00 = 3,072 bytes) in defkeymap.c: Already stubbed (accent_table_size=0)
   - Large functions: siphash_4u64 (1865 bytes), begin_new_exec (1926 bytes), balance_dirty_pages (631 bytes)

5. Subsystem sizes:
   - drivers/rtc: 412 LOC, actively used (CONFIG_RTC_LIB=y)
   - kernel/sched: 11,076 LOC, critical
   - drivers/tty: 1012KB, deeply integrated

6. Syscalls:
   - sys_ni.c (478 LOC): Already contains conditional syscall stubs

Conclusion: Most low-hanging fruit has been picked. Next steps require more aggressive approaches:
- Header stubbing (risky, arch dependencies)
- Subsystem-level simplification (TTY, VT, scheduler)
- Comment removal (doesn't reduce actual code complexity)
- Large file simplification (mm/page_alloc.c, fs/namei.c)

Current progress: -24 LOC this session (page_size_string removal only)
