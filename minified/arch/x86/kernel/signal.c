...
#define unsafe_put_sigcontext(sc,fp,regs,set,label) do { if (__unsafe_setup_sigcontext(sc, fp, regs, set->sig[0])) goto label; } while(0);

#define unsafe_put_sigmask(set,frame,label) unsafe_put_user(*(__u64 *)(set), (__u64 __user *)&(frame)->uc.uc_sigmask, label)
