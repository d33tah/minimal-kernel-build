...
#define _wait_rcu_gp(checktiny, ...) do {										call_rcu_func_t __crcu_array[] = { __VA_ARGS__ };			struct rcu_synchronize __rs_array[ARRAY_SIZE(__crcu_array)];		__wait_rcu_gp(checktiny, ARRAY_SIZE(__crcu_array),					__crcu_array, __rs_array);			} while (0)
