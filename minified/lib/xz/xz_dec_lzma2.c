			      ...
			      uint32_t *left)
{
	...
	while (*left > 0 && b->in_pos < b->in_size &&
		...
		copy_size =
			min(b->in_size - b->in_pos, b->out_size - b->out_pos);
...
static void lzma_rep_match(struct xz_dec_lzma2 *s, uint32_t pos_state)
{
	...
	if (!rc_bit(&s->rc, &s->lzma.is_rep0[s->lzma.state])) {
		if (!rc_bit(&s->rc,
			    &s->lzma.is_rep0_long[s->lzma.state][pos_state])) {
...
static bool lzma_main(struct xz_dec_lzma2 *s)
{
	...
	while (dict_has_space(&s->dict) && !rc_limit_exceeded(&s->rc)) {
		...
		if (!rc_bit(&s->rc,
			    &s->lzma.is_match[s->lzma.state][pos_state])) {
			lzma_literal(s);
		} else {
...
XZ_EXTERN enum xz_ret xz_dec_lzma2_run(struct xz_dec_lzma2 *s, struct xz_buf *b)
{
	...
	while (b->in_pos < b->in_size || s->lzma2.sequence == SEQ_LZMA_RUN) {
		...
		case SEQ_UNCOMPRESSED_1:
			s->lzma2.uncompressed += (uint32_t)b->in[b->in_pos++]
						 << 8;
		...
		case SEQ_LZMA_RUN:
			...
			dict_limit(&s->dict,
				   min_t(size_t, b->out_size - b->out_pos,
					 s->lzma2.uncompressed));
			if (!lzma2_lzma(s, b))
				return XZ_DATA_ERROR;
			...
			if (s->lzma2.uncompressed == 0) {
				if (s->lzma2.compressed > 0 ||
				    s->lzma.len > 0 || !rc_is_finished(&s->rc))
					return XZ_DATA_ERROR;
