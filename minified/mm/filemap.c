...
repeat: folio = mapping_get_entry(mapping, index);
...
no_page: if (!folio && (fgp_flags & FGP_CREAT)) {
		int err;
...
unlock_mapping: filemap_invalidate_unlock_shared(mapping);
...
static int filemap_create_folio(struct file *file, struct address_space *mapping, pgoff_t index, struct folio_batch *fbatch) {
	...
	filemap_invalidate_lock_shared(mapping);
	error = filemap_add_folio(mapping, folio, index, mapping_gfp_constraint(mapping, GFP_KERNEL));
	if (error == -EEXIST)
		error = AOP_TRUNCATED_PAGE;
	if (error)
		goto error;
	...
	return 0;
error: filemap_invalidate_unlock_shared(mapping);
	folio_put(folio);
	return error; }
...
retry: if (fatal_signal_pending(current))
		return -EINTR;
	...
	do {
		...
		for (i = 0; i < folio_batch_count(&fbatch); i++) {
			...
			if (copied < bytes) {
				error = -EFAULT;
				break; } }
put_folios: for (i = 0; i < folio_batch_count(&fbatch); i++)
...
ssize_t __generic_file_write_iter(struct kiocb *iocb, struct iov_iter *from) {
	...
	if (likely(written > 0))
		iocb->ki_pos += written;
out: return written ? written : err; }
