...
#define prefetchw_prev_lru_page(_page,_base,_field) do { if ((_page)->lru.prev != _base) { struct page *prev; prev = lru_to_page(&(_page->lru)); prefetchw(&prev->_field); } } while (0)
